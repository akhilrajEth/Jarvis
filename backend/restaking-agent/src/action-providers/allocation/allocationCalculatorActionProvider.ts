...
export class AllocationCalculatorActionProvider extends ActionProvider {
  ...
  @CreateAction({
    ...
    description: `Retrieves max allocation percent of ETH towards staking. If allocation percent is 65% and user has 1 ETH, you only use 0.65 to deposit staking.`,
    schema: FetchAllocationSchema,
  })
  async fetchAllocationAmount(): Promise<string> {
    console.log("Fetching max allocation for eth staking/restaking...");
    try {
      ...
      const stakePercentage = `${data.restaked_eth}%`;
      const resultMessage = `max eth allocation for staking/restaking is ${stakePercentage}`;
